...
import { buildItemPayload, itemToFormValues } from '../../utils/itemFormUtils.js';
import {
  FieldRow,
...
const UpdateItem = ({ item }) => {
  ...
  useEffect(() => {
    ...
    const init = itemToFormValues(item);
    if (init.quantityUnit === undefined) init.quantityUnit = item.quantityUnit || 'items';
    if (init.quantity === undefined && item.quantity !== undefined) init.quantity = item.quantity;
    setValues(init);
  ...
  const handleSubmit = async () => {
    ...
    if (!validateRequired() || !item?._id) return;

    const merged = { ...values, image: imagePath || values.image };
    const flat = buildItemPayload(merged, category);

    if (flat.expirationDate) {
      flat.expirationDate = moment(flat.expirationDate).toISOString();
    }
    if (flat.lastMaintenance) {
      flat.lastMaintenance = moment(flat.lastMaintenance).toISOString();
    }
    if (flat.calibration) {
      flat.calibration = moment(flat.calibration).toISOString();
    }

    try {
      await updateItem({ id: item._id, ...flat }).unwrap();
      message.success('Item updated!');
      setVisible(false);
    } catch (err) {
      console.error(err);
      const serverMsg = err?.data?.error || err?.error || err?.message;
      message.error(serverMsg ? `Failed to update: ${serverMsg}` : 'Failed to update item.');
    }
  };
  ...
  return (
    <div>
      <StyledButton type="secondary" icon={<EditOutlined />} size="default" onClick={() => setVisible(true)}>
        Update
      </StyledButton>
      <Modal
        title="Update item"
